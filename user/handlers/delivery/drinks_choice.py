...
from user.keyboards import create_drinks_kb, kb_order_more, create_tea_kb, create_fruit_tea_kb, create_milk_kb
from user.states import OrderClass
from database.functions.db_positions_lists import drinks_menu


@dp.message_handler(text='Чай (330 мл) 27 грн', state=OrderClass.Order_milk)
...
@dp.message_handler(text='Фруктовый Чай (330 мл) 37 грн', state=OrderClass.Order_milk)
async def choose_full_drink(message: Message, state: FSMContext):
    """ Processes simple tea. On ending sends taste-choice keyboard"""
...
async def choose_full_drink(message: Message, state: FSMContext):
    ...
    await OrderClass.Order_dessert.set()


@dp.message_handler(Text(equals=drinks_menu()), state=OrderClass.Order_milk)
async def choose_full_drink(message: Message, state: FSMContext):
...
@dp.message_handler(content_types=['text'], state=OrderClass.Order_milk)
async def choose_full_drink(message: Message):
...
@dp.message_handler(Text(equals=milks_menu()), state=OrderClass.Order_more)
...
@dp.message_handler(Text(equals=tea_menu()), state=OrderClass.Order_more)
async def process_drink_order(message: Message, state: FSMContext):
    """Processes milk choice. Calculates price. On ending sends more-drinks-choice keyboard"""
    chosen_milk = message.text
    data = await state.get_data()
...
@dp.message_handler(Text(equals=fruit_tea_menu()), state=OrderClass.Order_more)
async def process_drink_order(message: Message, state: FSMContext):
    """Processes milk choice. Calculates price. On ending sends more-drinks-choice keyboard"""
    chosen_milk = message.text
    data = await state.get_data()
...
@dp.message_handler(content_types=['text'], state=OrderClass.Order_more)
